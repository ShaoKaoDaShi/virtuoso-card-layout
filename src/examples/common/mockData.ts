...
// 生成示例虚拟列表数据
export const generateListItems = (count: number, repoId?: string): VirtualListItem[] => {
    ...
    content: `
        <div style="padding: 8px;">
          ...
          <div style="font-family: 'Monaco', 'Menlo', monospace; font-size: 13px; color: #666;">
            ${index % 3 === 0 ? "function calculateSum(a, b) {" : index % 3 === 1 ? "  return a + b;" : "}"}
          </div>
        </div>
      `,
    height: 60,
...
// 生成示例卡片数据
export const generateCards = (count: number, repoId?: string): CardData[] => {
